document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- EIP-6963 Provider Discovery (multi-injected detection) ---
...
async function connectViaWalletConnect() {
  if (!window.WalletConnectProvider) {
    throw new Error("WalletConnect script not loaded. Make sure ethereum-provider is included.");
  }
  if (!wcProvider) {
...
async function connectWallet(type, discoveredProviders) {
  ...
  if (injected) {
    try {
      ...
      console.warn(`${type} injected failed, fallback to WalletConnect…`, err);
      return await connectViaWalletConnect();
    }
  }
  return await connectViaWalletConnect();
